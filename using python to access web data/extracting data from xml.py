...
Actual data: http://py4e-data.dr-chuck.net/comments_603410.xml (Sum ends with 79)
...
also work from the top of the XML down to the comments node and then loop through the child nodes of the comments node.
Sample Execution
...
xml = urllib.request.urlopen(url).read() # request to open and read through url
print("Retrieved ", len(xml), " characters")

tree = ET.fromstring(xml)
total = 0
count = 0
for comments in tree.findall("comments"): # looking for comments in xml
    for comment in comments.findall("comment"): # finding comment in comments tree
        num = comment.find("count").text # finding count
        total += int(num)
        count +=1
