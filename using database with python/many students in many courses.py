...
Once you have made the necessary changes to the program and it has been run successfully reading the above J
SON data, run the following SQL command:
...
SELECT 'X' || hex(User.name || Course.title || Member.role ) AS X FROM
    ...
    ON User.id = Member.user_id AND Member.course_id = Course.id
    ORDER BY X LIMIT 1;
You should get one row with a string that looks like X53656C696E613333.
"""
import json
import sqlite3
...
result = ("""SELECT 'XYZZY' || hex(User.name || Course.title || Member.role ) AS X FROM 
    User JOIN Member JOIN Course 
    ON User.id = Member.user_id AND Member.course_id = Course.id
    ORDER BY X LIMIT 1""")
for test in cur.execute(result):
    print(test)
